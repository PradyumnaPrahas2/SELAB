...
public class TSP{
    ...
    public static void backtrack(int[][] adj,int n,int source,int node,boolean[] visited,int count,int cost){
        ...
        for(int i=0;i<n;i++){
            if(adj[node][i]>0){
                if(visited[i]==true && i==source && count==n-1){
                    ans=Math.min(ans,cost+adj[node][i]);
                }
                backtrack(adj,n,source,i,visited,count+1,cost+adj[node][i]);
            }
        }
        visited[node]=false;
    }
    
    public static int minCost(int[][] adj,int n){
        
        backtrack(adj,n,0,0,new boolean[n],0,0);
    ...
    public static void main (String[] args) {
        ...
        System.out.print(minCost(adj,n));
    }
}